 ...
 * @author fissban
 */
public class NpcVoteRewardNetwork extends AbstractMods
{
	...
	public boolean onInteract(Player player, Creature npc)
	{
		...
		hb.append(Html.headHtml("VOTE REWARD NETWORK"));
		hb.append("<br>");
		hb.append("<center>");
	...
	public void onEvent(Player player, Creature npc, String command)
	{
		...
		if (command.equals("vote"))
		{
			try
			{
				_votes = VoteReward.getVotesNetwork();
	...
	public void onTimer(String timerName, Npc npc, Player player)
	{
		...
		{
			case "waitVote":
				if (VoteReward.getVotesNetwork() > _votes)
				{
