...
import com.l2jserver.datapack.custom.engine.holders.BuffHolder;
...
public class SchemeBuffData
{
	...
	private static void loadBuffs()
	{
		try
		{
			File f = new File("./data/engine/scheme_buffer/generalBuffs.xml");
			...
			for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
			{
				if (d.getNodeName().equalsIgnoreCase("buff"))
				{
					...
					_generalBuffs.add(new BuffHolder(type, id, lvl, description));
				}
			}
	...
	private static void loadMageBuffs()
	{
		try
		{
			File f = new File("./data/engine/scheme_buffer/setMageBuffs.xml");
			//Document doc = XMLDocumentFactory.getInstance().loadDocument(f);
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			Document doc = factory.newDocumentBuilder().parse(f);
	...
	private static void loadWarriorBuffs()
	{
		try
		{
			File f = new File("./data/engine/scheme_buffer/setWarriorBuffs.xml");
			//Document doc = XMLDocumentFactory.getInstance().loadDocument(f);
			DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
			Document doc = factory.newDocumentBuilder().parse(f);
